    ...
    return (
      <li className="rounded-2xl bg-[#0D1828] border border-line border-[#111c2d] w-full">
        <button
          ...
          onClick={toggleOpen}
        >
          <span className="text-lg font-medium text-white w-[80%]">{item.question}</span>
          <svg
            className={`w-6 h-6 text-gray-400 transform transition-transform duration-200 ease-in-out ${isOpen ? 'rotate-180' : ''}`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
        ...
        >
          <p className='text-left max-w-xl p-4'>{item.answer}</p>
        </div>
      </li>
    )
  }
